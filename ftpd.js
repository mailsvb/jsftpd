...
class ftpd {
    constructor (options) {
        // options
        this._opt = {}
        this._opt.tls = Object.assign({}, TLSserverDefaults, options.tls)
        this._opt.cnf = Object.assign({}, FTPdefaults, options.cnf)
        ...
        // setup FTP on TLS
        this._tls = tls.Server(this._opt.tls)
    ...
    start () {
        ...
        this._tls.listen(this._opt.cnf.securePort)
    }
    ...
    Handler (main, socket) {
        ...
        const QUIT = function (cmd, arg) {
            main._writeToSocket(socket, '221', ' ', 'Goodbye', connectionInfo, SocketStateAfterWrite.Open)
            socket.end()
        }
        ...
        const SIZE = function (cmd, arg) {
            const file = path.normalize(basefolder + arg)
        ...
        const RMD = function (cmd, arg) {
            ...
            if (fs.existsSync(folder) === true && fs.statSync(folder).isDirectory() === true && main._beginsWith(basefolder, folder) === true) {
                if (allowFolderDelete) {
                    // TODO: remove rmDirSync when node 12 is deprecated
                    if (fs.rmSync) {
        ...
        const MKD = function (cmd, arg) {
            ...
            if (fs.existsSync(folder) === false && main._beginsWith(basefolder, folder) === true) {
            ...
            dataObj.method = function (obj) {
                if (obj.dataSocket && obj.cmdSocket && obj.absolutePath) {
                    ...
                    for (let i = 0; i < read.length; i++) {
                        let line
                        const file = path.normalize(obj.absolutePath + read[i].trim())
            ...
            if (fs.existsSync(file) === true && fs.statSync(file).isFile() === true && main._beginsWith(basefolder, file) === true) {
                dataObj.method = function (obj) {
                    if (obj.dataSocket && obj.cmdSocket && obj.file && obj.relativeFile) {
                        const streamOpts = {}
                        streamOpts.start = retrOffset
                        if (asciiOn) {
                            obj.dataSocket.setEncoding('ascii')
                            streamOpts.encoding = 'ascii'
                        }
        ...
        const validateLoginType = function () {
            ...
            } else if (username === main._opt.cnf.username && main._opt.cnf.allowLoginWithoutPassword === true) {
